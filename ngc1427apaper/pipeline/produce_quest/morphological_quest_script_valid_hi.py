...
def compute_hi_extrema(dff, idxs):
...
def compute_quest(dff, chunk, size):
    ...
    df.to_pickle(f'../quest/hi/hi_extrema.{chunk:02}.pkl')
    from astropy.table import Table
    tbl = Table.from_pandas(df)
    tbl.write(f'../quest/hi/hi_extrema.{chunk:02}.fits', overwrite=True)
